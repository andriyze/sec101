    ...
    <div className="animate-fade-in">
      ...
      <section className="section">
        <div className="section-title" style={{ marginBottom: '1rem' }}>
          ...
          <h3 style={{ margin: 0 }}>{t('phishing.red_flags.title')}</h3>
        </div>
      ...
      <section className="section">
        <div className="section-title" style={{ marginBottom: '1rem' }}>
          ...
          <h3 style={{ margin: 0 }}>{t('phishing.scams.title')}</h3>
        </div>
        <p style={{ marginBottom: '1rem', color: 'var(--text-muted)' }}>
          {t('phishing.scams.intro')}
        </p>
        <div className="grid grid-cols-3 gap-6">
          {tArray(t, 'phishing.scams.items').map(item => (
            <div
              key={item.title}
              className="card"
        ...
        <div className="alert alert-warning" style={{ marginTop: '1.5rem' }}>
          ...
          <div className="alert-content">
            <h3>{t('phishing.sextortion.title')}</h3>
            <p>{t('phishing.sextortion.intro')}</p>
            <ol style={{ paddingLeft: '1.2rem', marginTop: '0.5rem' }}>
              {tArray(t, 'phishing.sextortion.steps').map((item, i) => (
                <li key={i} style={{ marginBottom: '0.35rem' }}>
                  {item}
                </li>
              ))}
      ...
      <section className="section">
        <div className="section-title" style={{ marginBottom: '1rem' }}>
          ...
          <h3 style={{ margin: 0 }}>{t('phishing.checklist.title')}</h3>
        </div>
        <div className="card panel-solid" style={{ marginBottom: '1.5rem' }}>
          <ul className="list-clean">
            {tArray(t, 'phishing.checklist.items').map((item, i) => (
              <li
                key={i}
                style={{ display: 'flex', alignItems: 'start', gap: '1rem', marginBottom: '1rem' }}
              >
      ...
      <section className="section">
        <div className="section-title" style={{ marginBottom: '1rem' }}>
          ...
          <h3 style={{ margin: 0 }}>{t('phishing.other_channels.title')}</h3>
        </div>
        <p style={{ marginBottom: '1rem', color: 'var(--text-muted)' }}>
          {t('phishing.other_channels.intro')}
        </p>
        <div className="grid grid-cols-3 gap-6">
          <div className="card" style={{ borderLeft: '4px solid var(--secondary)' }}>
            <h4 style={{ color: 'var(--secondary)', marginBottom: '0.5rem' }}>
              {t('phishing.other_channels.smishing.title')}
            </h4>
            <p>{t('phishing.other_channels.smishing.desc')}</p>
          </div>
          <div className="card" style={{ borderLeft: '4px solid var(--secondary)' }}>
            <h4 style={{ color: 'var(--secondary)', marginBottom: '0.5rem' }}>
              {t('phishing.other_channels.vishing.title')}
            </h4>
            <p>{t('phishing.other_channels.vishing.desc')}</p>
          </div>
          <div className="card" style={{ borderLeft: '4px solid var(--secondary)' }}>
            <h4 style={{ color: 'var(--secondary)', marginBottom: '0.5rem' }}>
              {t('phishing.other_channels.quishing.title')}
            </h4>
