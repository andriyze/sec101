  ...
  const stepExplanations = [
    {
      ...
      detail: t(
        'visualizations.dns.steps.1.detail',
        'When you type "google.com", your browser first checks its cache. If not found, it asks your configured DNS resolver.'
      ),
      serverRole: t(
        'visualizations.dns.steps.1.role',
        'DNS Resolver: A server that finds IP addresses for you. Like a librarian who knows where to find information.'
      ),
    ...
    },
    {
      ...
      description: t(
        'visualizations.dns.steps.2.short',
        'Resolver asks: "Who handles .com domains?"'
      ),
      detail: t(
        'visualizations.dns.steps.2.detail',
        'The resolver starts at the top of DNS hierarchy - the 13 root server clusters worldwide.'
      ),
      serverRole: t(
        'visualizations.dns.steps.2.role',
        'Root Server: The starting point for all DNS queries. Knows which servers handle each top-level domain.'
      ),
    ...
    },
    {
      ...
      description: t(
        'visualizations.dns.steps.3.short',
        'TLD server knows who controls google.com'
      ),
      detail: t(
        'visualizations.dns.steps.3.detail',
        'The .com TLD server manages all .com registrations and knows which authoritative servers handle each domain.'
      ),
      serverRole: t(
        'visualizations.dns.steps.3.role',
        'TLD Server: Manages a domain extension like .com.'
      ),
    ...
    },
    {
      ...
      detail: t(
        'visualizations.dns.steps.4.detail',
        "Google's authoritative name server has the definitive answer because Google controls it."
      ),
      serverRole: t(
        'visualizations.dns.steps.4.role',
        'Authoritative Server: The final source of truth. Controlled by the domain owner.'
      ),
    ...
    },
    {
      ...
      detail: t(
        'visualizations.dns.steps.5.detail',
        'The IP travels back through the resolver and gets cached. Your browser can now connect directly.'
      ),
      serverRole: t(
        'visualizations.dns.steps.5.role',
        'Complete: DNS resolution done in 20-120 milliseconds.'
      ),
  ...
  const nodes = [
    {
      id: 'device',
      icon: Monitor,
      label: t('visualizations.dns.your_device'),
      color: 'var(--primary)',
    },
    {
      id: 'resolver',
      icon: Server,
      label: t('visualizations.dns.resolver'),
      color: 'var(--secondary)',
    },
    ...
    >
      <div className="dns-viz-wrapper">
        ...
        <details className="dns-technical">
          ...
          <p>
            {t(
              'visualizations.dns.technical_text',
              'DNS usually uses UDP port 53 for queries, but TCP port 53 is also part of DNS. The TTL (Time To Live) determines how long a result is cached. DoT uses TLS on port 853; DoH sends DNS over HTTPS on port 443. Encrypted DNS protects the client-to-resolver hop, but the resolver still sees the query.'
            )}
          </p>
        </details>
      </div>
